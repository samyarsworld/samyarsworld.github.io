...
function resizeGrid(size) {
  let rows =
    height > width
      ? Math.floor(grid.offsetHeight / size)
      : Math.floor(
          (height - document.querySelector(".buttons-container").offsetHeight) /
            size
        );
...
function generateGrid() {
  ...
  let rows =
    height > width
      ? Math.floor(grid.offsetHeight / NODE_SIZE)
      : Math.floor(
          (height - document.querySelector(".buttons-container").offsetHeight) /
            NODE_SIZE
        );
...
function divHover(e) {
  ...
  if (
    ...
    e.target.classList.contains("node-wall") ||
    e.target.classList.contains("node-start") ||
    e.target.classList.contains("node-end")
  ) {
    return;
  }
  if (startNode && endNode) {
