...
function doDijkstra(
  ...
  if (realtime) {
    ...
    setTimeout(() => {
      div.classList.remove(`node-current${type}`);
      div.classList.add(`node-check${type}`);
    }, 1000);
  }
  processed.add(curr);
...
function doAStar(
  ...
  minHeap.sort(
    (a, b) =>
      distanceMap.get(b) +
      heursticMap.get(b) -
      (distanceMap.get(a) + heursticMap.get(a))
  );
  let curr = minHeap.pop();
  let div = curr.divReference;
  if (processed.has(curr)) {
...
function checkIntersectionAStar(
  processed,
  curr,
  currentParentMap,
  otherParentMap
...
function checkIntersectionAStarRT(
  processed,
  curr,
  currentParentMap,
  otherParentMap
  ...
  for (let i = 0; i < grid.length; ++i) {
    for (let j = 0; j < grid[i].length; ++j) {
      ...
      forwardHeuristic.set(
        grid[i][j],
        Math.abs(grid[i][j].row - end.row) + Math.abs(grid[i][j].col - end.col)
      );
      backwardHeuristic.set(
        grid[i][j],
        Math.abs(grid[i][j].row - start.row) +
          Math.abs(grid[i][j].col - start.col)
      );
    }
  }
  forwardDistanceMap.set(forwardCurr, 0);
