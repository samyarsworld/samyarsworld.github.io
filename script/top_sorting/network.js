document
  .querySelector("#task-adder")
  .addEventListener("click", generateFormInput);
document
  .querySelector("#network-visualizer")
...
function visualizeNetwork(e) {
  e.preventDefault();
  document.getElementById("result").scrollIntoView({ behaviour: "smooth" });
...
function topologicalSort(adjMap, inDegreeMap) {
  ...
  document.getElementById("result").textContent = answer.toString();
  document.getElementById("result").className = "lead";
}
